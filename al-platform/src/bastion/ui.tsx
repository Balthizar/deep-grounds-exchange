  ...
  return (
    <>
      ...
      {fac.defId === "scriptorium" && !frozen && (() => {
        ...
        // ⚠ THE CANDIDATES ARE REAL PEOPLE NOW (Frank, 3 Aug). This drew from three hardcoded arrays —
        // eight English first names, six gods, six Sword Coast towns — so a keep in Chult was offered
        // "Maribel, Acolyte of Chauntea" and "Corwin, Novice Mage of Daggerford". No species, no
        // surname, no naming culture, and nothing regional about any of it.
        //
        //   *"The person is generated by the hireling system, the classes are generated by the room
        //    and region... from the user's perspective these two people arrived to offer their
        //    services."*
        //
        // The ACOLYTE needs no new data: a person drawn from this region's demographics already
        // carries a `faith`, rolled from FAITH_BY_CULTURE against their own species. **The
        // demographics pick the people and the people pick the god.**
        //
        // The MAGE reads `REGION_ARCANE` — a real school where one exists, an outsider's training
        // where none does, and in Barovia no mage offers at all.
        const regionId = (ch.bastion && ch.bastion.region) || null;
        const localeId = (ch.bastion && ch.bastion.locale) || null;
        const mageComes = aMageOffers(regionId);
        // ⚠ AND THE ACOLYTE MUST BE SOMEBODY WHO PRAYS (Frank, 3 Aug). A githyanki was being drawn as
        // the divine candidate and would have offered as an acolyte of nothing. Redraw until the
        // people worships — the mage half has no such constraint, because anybody can be taught.
        const cands = classes
          .filter((c: any) => (c.cls !== "Wizard" || mageComes)
                           && (c.cls !== "Cleric" || anAcolyteOffers(regionId, localeId)))
          .map((c: any) => {
          let who = randSpecies(regionId, localeId, "hire", ch.bastion && ch.bastion.form, "scriptorium");
          let nmRec = randName(who.species);
          let person = rollPerson(who.species, nmRec, 20 + Math.floor(Math.random() * 22), "Scribe");
          // ⚠ AND THE ACOLYTE MUST ACTUALLY HOLD A FAITH, not merely belong to a people that can
          // (Frank, 3 Aug). The species filter passed a human through and **that human then rolled
          // "no god in particular" 21% of the time**, presenting as an acolyte of nothing much.
          //
          // Same shape as the githyanki, one level down: I filtered on *can this people pray*, not
          // *does this person*. **A temple sends a believer** — so the whole candidate is redrawn,
          // rather than rerolling their faith, which would quietly override the 12–44% "no god"
          // weight the faith tables carry on purpose.
          if (c.cls === "Cleric") {
            for (let i = 0; i < 30; i++) {
              const godly = person.faith && person.faith !== "no god in particular";
              if (canBeAcolyte(who.species) && godly) break;
              who = randSpecies(regionId, localeId, "hire", ch.bastion && ch.bastion.form, "scriptorium");
              nmRec = randName(who.species);
              person = rollPerson(who.species, nmRec, 20 + Math.floor(Math.random() * 22), "Scribe");
            }
          }
          const nm = nmRec.name;
          const tail = c.cls === "Cleric"
            ? "acolyte of " + person.faith
            : arcaneBlurb(regionId);
          return { id: c.id, name: nm, species: who.species,
                   blurb: nm + " \u2014 " + who.species + ", " + tail, cls: c.cls, label: c.label };
        });
        return (
          <div className="dg-lane">
